...
int threshold =45;          // Define the threshold between black and white colors (white ~ 50; black ~ 30)//
...
task main()
{
	while(true)        // (infinite loop, also represented by 'while(1)).
	{
		...
		{
			motor(leftmotor) = (leftlight - threshold) * 10;                    // leftmotor is run at a 20 power level.
			motor(rightmotor) = (rightlight - threshold) * 10;                   // rightmotor is run at a 20 power level.
		}
