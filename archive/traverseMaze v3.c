...
void drive() {
	...
	while(true)
	{
		...
		motor(leftmotor) = (leftlight - rightlight / 2 + 5) / 2;
		motor(rightmotor) = (rightlight - leftlight / 2 + 5) / 2;
...
void checkDir() {
	...
	if(sidedistance > 18){
		...
		while(rightlight > leftlight - thresholdDiff){
			readsensors();
		}
	...
	while(frontdistance < 25) {
		...
		while(leftlight > rightlight - thresholdDiff){
			readsensors();
		}
